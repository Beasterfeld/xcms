...
setGeneric( "broadenMzRanges", function(object, ...) standardGeneric("broadenMzRanges"))

setMethod( "broadenMzRanges", "xcmsSet", function( object, peakidx = NULL, max.ppm = 50 ){
  if( is.null( object@peaks ) ){
    stop( "Called `broadenMzRanges` but there is no peak table. Did you call findPeaks()?" )
  }
  
  # broadenMzRanges  <- function( object, peakidx = NULL, max.ppm = 50 ){
  # load("../rdpt/xset0.RData")
  # object <- xset; rm(xset)
  # peakidx = NULL
  # max.ppm = 50
  
  # get the peak table to process on on
  peaks <- object@peaks
  if( ! is.null( peakidx ) ) {
    peaks <- peaks[ peakidx, , drop = FALSE ]
  }
  
  # iterate over the samples
  # @TODO use ddply or data.table?
  for( sample in unique( peaks[ , "sample"] ) ){
    # get RT table mapping raw and corrected time
    rttab <- data.frame(
      raw       = object@rt$raw[[sample]],
      corrected = object@rt$corrected[[ sample ]]
    )
    raw <- xcmsRaw( object@filepaths[ sample ] )
    rttab <- rttab[ rttab$raw >= min( raw@scantime ) & rttab$raw <= max( raw@scantime ),  ]
    rttab <- as.matrix(rttab)
    
    # Should never happen
    if( nrow( rttab ) != length(raw@scantime) ){
      stop("Number of scans (", length(raw@scantime),") in sample ", object@filepaths[sample],
    ...
    #    scan    <- rep( raw@acquisitionNum, times = ts )
    # now build a table {int, mz} as efficient as possible
    # @TODO, {int} could even be left out getting a plain vector
    rawdata <- as.matrix( data.frame( int = raw@env$intensity, mz = raw@env$mz ) )
    ...
    for( k in which( peaks[ , "sample" ] == sample ) ){
      ...
      eic <- rawEIC( raw, mzrange = peaks[ k, c( 2,3 ) ], rtrange = rtrange  )
      scs <- length( eic$scan )
      # remove the first and the last scan of this eic, as they may be empty
      eic <- matrix( c( eic$scan[ -c(1, scs) ], eic$intensity[ -c(1, scs) ] ), ncol = 2 )
      # plot( as.data.frame(eic), type = "l" )
      
      # get the empty scans, except the first and the last
      emptyScans <- eic[ which( eic[ ,2] == 0 ), 1 ]
      
      # if there are no empty scans do nothing
      if( length(emptyScans) == 0 ){
        next;
      }
      
      # the maximum allowed range
      allowed.range <- peaks[ k, c("mzmin", "mzmax") ] * c( 1 - max.ppm * 1E-6, 1 + max.ppm * 1E-6  )
      broadenedMz <- numeric( length(emptyScans) )
      
      # now find for each scan the closest mz to peaks[ k, c("mzmin", "mzmax") ] within the allowed range
      for( i in seq_along(emptyScans) ){
        es <- emptyScans[ i ]
        # select by scan
        q <- rawdata[ (raw@scanindex[es]+1) : (raw@scanindex[es + 1]), 2 ]
        # select by mz range
        q <- q[ q >= allowed.range[1] & q <= allowed.range[2] ]
        if( length(q) == 1 ){
          broadenedMz[i] <- q
        } else if( length(q) > 1 ){
          # print(abs( q[ ,2, drop = FALSE] - peaks[ k, c("mzmin", "mzmax") ] ))
          m <- matrix( c( abs( q - peaks[ k, 2 ] ), abs( q - peaks[ k, 3 ] ) ), ncol = 2 )
          broadenedMz[i] <- min( apply( m , 1, min ) )
        }
        else{
          broadenedMz[i] <- NA
        }
      ...
      newRange <- range( c( peaks[ k, c(2,3) ], broadenedMz ), na.rm = TRUE )
      # eic <- as.data.frame( rawEIC( raw, mzrange = newRange, rtrange = rtrange  ) )
      # plot( as.data.frame(eic), type = "l" )
      
      # set the new range
      peaks[ k, c(2,3) ] <- newRange
    }
  }
  
  # reset the calculated mzranges
  object@peaks <- peaks
  return( object )
})
